...
compressor_c::~compressor_c() {
  ...
  mxverb(2,
         strformat::bstr("compression: Overall stats: raw size: %1%, compressed size: %2%, items: %3%, ratio: %|4$.2f|%%, avg bytes per item: %5%\n")
...
compressor_c::create_from_file_name(std::string const &file_name) {
  ...
  auto ext = mbalgm::to_lower_copy(pos == std::string::npos ? file_name : file_name.substr(pos + 1));
