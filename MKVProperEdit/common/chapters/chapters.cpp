...
chapter_error(const std::string &error) {
  throw parser_x(strformat::bstr(Y("Simple chapter parser: %1%\n")) % error);
}

inline void
chapter_error(const strformat::bstr &format) {
  ...
  while (in->getline2(line)) {
    ...
    if (0 == mode) {
      if (!std::regex_match(line, matches, timestamp_line_re))
        chapter_error(strformat::bstr(Y("'%1%' is not a CHAPTERxx=... line.")) % line);
      ...
      if (59 < minute)
        chapter_error(strformat::bstr(Y("Invalid minute: %1%")) % minute);
      if (59 < second)
        chapter_error(strformat::bstr(Y("Invalid second: %1%")) % second);
      ...
      if (!std::regex_match(line, matches, name_line_re))
        chapter_error(strformat::bstr(Y("'%1%' is not a CHAPTERxxNAME=... line.")) % line);
...
parse(const std::string &file_name,
  ...
  try {
    ...
    if (exception_on_error)
      throw;
    mxerror(strformat::bstr(Y("Could not parse the chapters in '%1%': %2%\n")) % file_name % e.error());

  } catch (...) {
    if (exception_on_error)
      throw parser_x(strformat::bstr(Y("Could not open '%1%' for reading.\n")) % file_name);
    else
      mxerror(strformat::bstr(Y("Could not open '%1%' for reading.\n")) % file_name);
  }
...
parse(mm_text_io_c *in,
  ...
  try {
    ...
    error = (strformat::bstr(Y("Unknown chapter file format in '%1%'. It does not contain a supported chapter format.\n")) % in->get_file_name()).str();
  } catch (mtx::chapters::parser_x &e) {
    error = e.error();
  } catch (mtx::mm_io::exception &ex) {
    error = (strformat::bstr(Y("The XML chapter file '%1%' could not be read.\n")) % in->get_file_name()).str();
  } catch (mtx::xml::xml_parser_x &ex) {
    error = (strformat::bstr(Y("The XML chapter file '%1%' contains an error at position %3%: %2%\n")) % in->get_file_name() % ex.result().description() % ex.result().offset).str();
  } catch (mtx::xml::exception &ex) {
    error = (strformat::bstr(Y("The XML chapter file '%1%' contains an error: %2%\n")) % in->get_file_name() % ex.what()).str();
  }
...
remove_entries(int64_t min_ts,
  ...
  for (i = 0; m.ListSize() > i; ++i) {
    ...
    mxverb(3,
           strformat::bstr("remove_chapters: entries[%1%]: remove %2% spans %3% start %4% end %5%\n")
...
merge_entries(EbmlMaster &master) {
  ...
  for (master_idx = 0; master.ListSize() > master_idx; ++master_idx) {
    ...
    mxverb(3, strformat::bstr("chapters: merge_entries: looking for %1% with %2%, %3%\n") % uid % start_ts % end_ts);
    ...
    while (true) {
      ...
      mxverb(3, strformat::bstr("chapters: merge_entries:   found one at %1% with %2%, %3%; merged to %4%, %5%\n") % merge_idx % merge_start_ts % merge_end_ts % start_ts % end_ts);
...
format_name_template(std::string const &name_template,
  ...
  name = std::regex_replace_lambda(name, number_re, [=](std::smatch const &match) {
    auto number_str    = (strformat::bstr("%1%") % chapter_number).str();
  ...
  name = std::regex_replace_lambda(name, timestamp_re, [=](std::smatch const &match) {
