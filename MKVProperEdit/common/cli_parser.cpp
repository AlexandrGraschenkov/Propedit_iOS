  ...
  for (auto sit = m_args.cbegin(), sit_end = m_args.cend(); sit != sit_end; sit++) {
    ...
    if (option_it != m_option_map.end()) {
      ...
      if (option.m_needs_arg) {
        if (no_next_arg)
          mxerror(strformat::bstr(Y("Missing argument to '%1%'.\n")) % m_current_arg);
        ++sit;
      }
      ...
      mxerror(strformat::bstr(Y("Unknown option '%1%'.\n")) % m_current_arg);
  }
}
...
parser_c::add_option(std::string const &spec,
  ...
  for (auto &name : names) {
    ...
    if (mtx::includes(m_option_map, full_name))
      mxerror(strformat::bstr("parser_c::add_option(): Programming error: option '%1%' is already used for spec '%2%' "
