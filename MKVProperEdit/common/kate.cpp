...
kate_parse_identification_header(const unsigned char *buffer,
  ...
  if (KATE_HEADERTYPE_IDENTIFICATION != header.headertype)
    throw mtx::kate::header_parsing_x(strformat::bstr(Y("Wrong header type: 0x%|1$02x| != 0x%|2$02x|")) % header.headertype % KATE_HEADERTYPE_IDENTIFICATION);

  for (i = 0; 7 > i; ++i)
    header.kate_string[i] = bc.get_bits(8);
  if (memcmp(header.kate_string, "kate\0\0\0", 7))
    throw mtx::kate::header_parsing_x(strformat::bstr(Y("Wrong identification string: '%|1$7s|' != 'kate\\0\\0\\0'")) % header.kate_string); /* won't print NULs well, but hey */
  ...
  if (header.vmaj > supported_version)
    throw mtx::kate::header_parsing_x(strformat::bstr(Y("Wrong Kate version: %1%.%2% > %3%.x")) % header.vmaj % header.vmin % supported_version);
