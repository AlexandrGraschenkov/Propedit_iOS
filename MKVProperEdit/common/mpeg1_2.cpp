...
mpeg1_2::extract_fps_idx(const unsigned char *buffer,
                         int buffer_size) {
  mxverb(3, strformat::bstr("mpeg_video_fps: start search in %1% bytes\n") % buffer_size);
  if (buffer_size < 8) {
  ...
  mxverb(3, strformat::bstr("mpeg_video_fps: found sequence header start code at %1%\n") % (idx - 4));
...
mpeg1_2::extract_ar(const unsigned char *buffer,
  ...
  mxverb(3, strformat::bstr("mpeg_video_ar: start search in %1% bytes\n") % buffer_size);
  if (buffer_size < 8) {
  ...
  mxverb(3, strformat::bstr("mpeg_video_ar: found sequence header start code at %1%\n") % (idx - 4));
