...
public:
  ...
  virtual std::string error() const throw() {
    return (strformat::bstr(Y("Creating directory '%1%' failed: %2%")) % m_path % code().message()).str();
  }
};
...
public:
  ...
  virtual std::string error() const throw() {
    return (strformat::bstr(Y("Invalid UTF-8 char. First byte: 0x%|1$02x|")) % static_cast<unsigned int>(m_first_char)).str();
  }
};
