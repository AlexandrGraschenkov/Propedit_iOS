...
theora_parse_identification_header(unsigned char *buffer,
  ...
  if (THEORA_HEADERTYPE_IDENTIFICATION != header.headertype)
    throw mtx::theora::header_parsing_x(strformat::bstr(Y("Wrong header type: 0x%|1$02x| != 0x%|2$02x|")) % header.headertype % THEORA_HEADERTYPE_IDENTIFICATION);

  for (i = 0; 6 > i; ++i)
    header.theora_string[i] = bc.get_bits(8);
  if (strncmp(header.theora_string, "theora", 6))
    throw mtx::theora::header_parsing_x(strformat::bstr(Y("Wrong identification string: '%|1$6s|' != 'theora'")) % header.theora_string);
  ...
  if ((3 != header.vmaj) || (2 != header.vmin))
    throw mtx::theora::header_parsing_x(strformat::bstr(Y("Wrong Theora version: %1%.%2%.%3% != 3.2.x")) % header.vmaj % header.vmin % header.vrev);
