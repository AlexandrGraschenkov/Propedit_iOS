...
static mbalgm::optional<std::size_t>
find_stop_display_position(unsigned char const *data,
  ...
  while ((start_off != next_off) && (next_off < buf_size)) {
    ...
    for (auto type = data[off++]; type != 0xff; type = data[off++]) {
      auto info    = (strformat::bstr("spu_extraction_duration: cmd = %|1$02x| ") % static_cast<unsigned int>(type)).str();
      auto unknown = false;
      switch(type) {
        case 0x00:
          // Menu ID, 1 byte
        ...
        case 0x02: {
          ...
          info     += (strformat::bstr("stop display: %1%\n") % format_timestamp(date)).str();
        ...
        case 0xff:
          ...
          return {};
        default:
          info += (strformat::bstr("unknown (0x%|1$02x|), skipping %2% bytes.") % type % (next_off - off)).str();
          unknown = true;
      }
      mxdebug_if(debug, strformat::bstr("%1%\n") % info);
      if (unknown)
        break;
    }
  }
