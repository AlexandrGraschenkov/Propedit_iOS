...
#include "common/compression/zlib_compression.h"
#include <zlib.h>
...
zlib_compressor_c::do_decompress(memory_cptr const &buffer) {
  ...
  if (Z_OK != result)
    mxerror(strformat::bstr(Y("inflateInit() failed. Result: %1%\n")) % result);
  ...
  do {
    ...
    if ((Z_OK != result) && (Z_STREAM_END != result))
      throw mtx::compression_x(strformat::bstr(Y("Zlib decompression failed. Result: %1%\n")) % result);
  ...
  mxverb(3, strformat::bstr("zlib_compressor_c: Decompression from %1% to %2%, %3%%%\n") % buffer->get_size() % dst->get_size() % (dst->get_size() * 100 / buffer->get_size()));
...
zlib_compressor_c::do_compress(memory_cptr const &buffer) {
  ...
  if (Z_OK != result)
    mxerror(strformat::bstr(Y("deflateInit() failed. Result: %1%\n")) % result);
  ...
  do {
    ...
    if ((Z_OK != result) && (Z_STREAM_END != result))
      mxerror(strformat::bstr(Y("Zlib decompression failed. Result: %1%\n")) % result);
  ...
  mxverb(3, strformat::bstr("zlib_compressor_c: Compression from %1% to %2%, %3%%%\n") % buffer->get_size() % dst->get_size() % (dst->get_size() * 100 / buffer->get_size()));
