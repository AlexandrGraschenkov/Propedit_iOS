  ...
  , m_from_utf8_handle(s_iconv_t_error_value)
{
  ...
  if (s_iconv_t_error_value == m_to_utf8_handle)
    mxwarn(strformat::bstr(Y("Could not initialize the iconv library for the conversion from %1% to UTF-8. "
  ...
  if (s_iconv_t_error_value == m_from_utf8_handle)
    mxwarn(strformat::bstr(Y("Could not initialize the iconv library for the conversion from UTF-8 to %1%. "
...
iconv_charset_converter_c::convert(iconv_t handle,
  ...
  iconv(handle, (char **)&ptr_source, &length_source, &ptr_destination, &length_destination);
