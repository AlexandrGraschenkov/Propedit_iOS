...
id_header_t::decode(memory_cptr const &mem) {
  if (!mem || (sizeof(packed_id_header_t) > mem->get_size()))
    throw decode_error{strformat::bstr("Size of memory too small: %1% < %2%") % (mem ? mem->get_size() : 0) % sizeof(packed_id_header_t)};
  ...
  if (identifier != "OpusHead")
    throw decode_error{strformat::bstr("Identifier not 'OpusHead': %1%") % identifier};
