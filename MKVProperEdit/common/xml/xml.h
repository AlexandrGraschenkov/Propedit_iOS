...
public:
  ...
  conversion_x(strformat::bstr const &message): m_message(message.str()) { }
...
public:
  invalid_attribute_x(std::string const &node, std::string const &attribute, ptrdiff_t position)
    ...
    , m_position(position)
  {
    m_message = (strformat::bstr(Y("Invalid attribute '%1%' in node '%2%' at position %3%")) % m_attribute % m_node % m_position).str();
  }
...
public:
  invalid_child_node_x(std::string const &node, std::string const &parent, ptrdiff_t position)
    ...
    , m_position(position)
  {
    m_message = (strformat::bstr(Y("<%1%> is not a valid child element of <%2%> at position %3%.")) % m_node % m_parent % m_position).str();
  }
...
public:
  duplicate_child_node_x(std::string const &node, std::string const &parent, ptrdiff_t position)
    ...
    , m_position(position)
  {
    m_message = (strformat::bstr(Y("Only one instance of <%1%> is allowed beneath <%2%> at position %3%.")) % m_node % m_parent % m_position).str();
  }
...
public:
  malformed_data_x(std::string const &node, ptrdiff_t position, std::string const &details = std::string{})
    ...
    , m_position(position)
  {
    m_message = (strformat::bstr(Y("The tag or attribute '%1%' at position %2% contains invalid or mal-formed data.")) % m_node % m_position).str();
    if (!details.empty())
      m_message += " " + details;
  }
...
public:
  out_of_range_x(std::string const &node, ptrdiff_t position, std::string const &details = std::string{})
    ...
    , m_position(position)
  {
    m_message = (strformat::bstr(Y("The tag or attribute '%1%' at position %2% contains data that is outside its allowed range.")) % m_node % m_position).str();
    if (!details.empty())
      m_message += " " + details;
  }
...
document_cptr load_file(std::string const &file_name, unsigned int options = pugi::parse_default, mbalgm::optional<int64_t> max_read_size = mbalgm::optional<int64_t>{});
