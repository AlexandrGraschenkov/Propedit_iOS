...
ebml_segmentinfo_converter_c::parse_file(std::string const &file_name,
  ...
  try {
    ...
    mxerror(strformat::bstr(Y("The XML segmentinfo file '%1%' could not be read.\n")) % file_name);

  } catch (mtx::xml::xml_parser_x &ex) {
    mxerror(strformat::bstr(Y("The XML segmentinfo file '%1%' contains an error at position %3%: %2%\n")) % file_name % ex.result().description() % ex.result().offset);

  } catch (mtx::xml::exception &ex) {
    mxerror(strformat::bstr(Y("The XML segmentinfo file '%1%' contains an error: %2%\n")) % file_name % ex.what());
  }
