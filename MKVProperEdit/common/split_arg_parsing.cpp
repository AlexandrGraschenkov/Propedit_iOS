...
parse_split_parts(const std::string &arg,
  ...
  if (s.empty())
    throw format_x{strformat::bstr(Y("Missing start/end specifications for '--split' in '--split %1%'.\n")) % arg};
  ...
  for (auto const &part_spec : split(s, ",")) {
    ...
    if (pair.size() != 2)
      throw format_x{strformat::bstr(Y("Invalid start/end specification for '--split' in '--split %1%' (current part: %2%).\n")) % arg % part_spec};
    ...
    if (pair[0].empty())
      ...
      throw format_x{strformat::bstr(Y("Invalid start time for '--split' in '--split %1%' (current part: %2%). Additional error message: %3%.\n")) % arg % part_spec % timestamp_parser_error};

    else if (frames_fields && (!parse_number(pair[0], start) || (0 > start)))
      throw format_x{strformat::bstr(Y("Invalid start frame/field number for '--split' in '--split %1%' (current part: %2%).\n")) % arg % part_spec};
    ...
    if (pair[1].empty())
      ...
      throw format_x{strformat::bstr(Y("Invalid end time for '--split' in '--split %1%' (current part: %2%). Additional error message: %3%.\n")) % arg % part_spec % timestamp_parser_error};

    else if (frames_fields && (!parse_number(pair[1], end) || (0 > end)))
      throw format_x{strformat::bstr(Y("Invalid end frame/field number for '--split' in '--split %1%' (current part: %2%).\n")) % arg % part_spec};

    if (end <= start) {
      if (frames_fields)
        throw format_x{strformat::bstr(Y("Invalid end frame/field number for '--split' in '--split %1%' (current part: %2%). The end number must be bigger than the start number.\n")) % arg % part_spec};
      else
        throw format_x{strformat::bstr(Y("Invalid end time for '--split' in '--split %1%' (current part: %2%). The end time must be bigger than the start time.\n")) % arg % part_spec};
    }

    if (!requested_split_points.empty() && (start < std::get<1>(requested_split_points.back()))) {
      if (frames_fields)
        throw format_x{strformat::bstr(Y("Invalid start frame/field number for '--split' in '--split %1%' (current part: %2%). The start number must be bigger than or equal to the previous part's end number.\n")) % arg % part_spec};
      else
        throw format_x{strformat::bstr(Y("Invalid start time for '--split' in '--split %1%' (current part: %2%). The start time must be bigger than or equal to the previous part's end time.\n")) % arg % part_spec};
    }
