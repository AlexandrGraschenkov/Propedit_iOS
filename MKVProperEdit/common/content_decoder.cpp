...
content_decoder_c::initialize(KaxTrackEntry &ktentry) {
  ...
  for (auto kcenc_el : *kcencodings) {
    ...
    if (1 == enc.type) {
      mxwarn(strformat::bstr(Y("Track number %1% has been encrypted and decryption has not yet been implemented.\n")) % tid);
      ok = false;
      break;
    }

    if (0 != enc.type) {
      mxerror(strformat::bstr(Y("Unknown content encoding type %1% for track %2%.\n")) % enc.type % tid);
      ok = false;
      break;
    }
    ...
    if (0 == enc.comp_algo)
      ...
      mxwarn(strformat::bstr(Y("Track %1% was compressed with the algorithm '%2%' which is not supported anymore.\n")) % tid % algorithm);
      ok = false;
      break;
      ...
      mxwarn(strformat::bstr(Y("Track %1% has been compressed with an unknown/unsupported compression algorithm (%2%).\n")) % tid % enc.comp_algo);
      ok = false;
      break;
    }
